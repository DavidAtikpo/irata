...
export function generateTraineeFollowUpPDF(
  ...
  const headers = ['Éléments du programme', ...levels, ...trainees.flatMap(trainee => days.map(day => `${trainee.prenom} ${trainee.nom} - ${day}`))];
  ...
  autoTable(doc, {
    ...
    didParseCell: function(data) {
      // Colorer les cellules des stagiaires
  ...
  // Section des signatures des stagiaires
  let traineesSignatureY = 0;
  autoTable(doc, {
    ...
    didDrawPage: function(data) {
      if (data.cursor) {
        traineesSignatureY = data.cursor.y;
      }
    }
  });

  // Ajouter les signatures des stagiaires
  let currentY = traineesSignatureY + 10;
  trainees.forEach((trainee, index) => {
    const signature = signatures.find(s => s.traineeId === trainee.id)?.signature;
    
    doc.setFontSize(10);
    doc.setFont('helvetica', 'bold');
    doc.text(`${trainee.prenom} ${trainee.nom}:`, 20, currentY);
    ...
    if (signature) {
      try {
        ...
        doc.addImage(signature, 'PNG', 80, currentY - 5, 30, 15);
        doc.text('✓ Signé', 120, currentY + 5);
      } catch (error) {
...
export function generateTraineeIndividualPDF(
  ...
  autoTable(doc, {
    ...
    didDrawPage: function(data) {
      if (data.cursor) {
        individualSignatureY = data.cursor.y;
      }
    }
  });
