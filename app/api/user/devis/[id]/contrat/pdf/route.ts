...
export async function GET(
  ...
  try {
    ...
    y -= 15;
    page.drawText(`Email : ${devis.demande.user.email}`, { x: 70, y, size: 10, font, color: rgb(0,0,0) });
    ...
    y -= 20;
    page.drawText(`Formation Cordiste IRATA - ${devis.demande.session}`, { x: 70, y, size: 10, font, color: rgb(0,0,0) });
    y -= 15;
    page.drawText(`Montant : ${(devis.montant || 0).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' ')} € net`, { x: 70, y, size: 10, font, color: rgb(0,0,0) });
    y -= 15;
    page.drawText(`Date de formation : ${devis.dateFormation ? new Date(devis.dateFormation).toLocaleDateString('fr-FR') : 'Non définie'}`, { x: 70, y, size: 10, font, color: rgb(0,0,0) });
    ...
    y -= 20;
    page.drawText(`Statut : ${contrat.statut}`, { x: 50, y, size: 10, font: boldFont, color: rgb(0,0,0) });
    ...
    if (y > 200) {
      y -= 40;
      page.drawText('Articles du contrat :', { x: 50, y, size: 12, font: boldFont, color: rgb(0,0,0) });
      y -= 20;
      page.drawText('• Formation de 5 jours + 1 jour d\'examen', { x: 70, y, size: 9, font, color: rgb(0,0,0) });
      y -= 12;
      page.drawText('• Certification IRATA', { x: 70, y, size: 9, font, color: rgb(0,0,0) });
      y -= 12;
      page.drawText('• Hébergement inclus', { x: 70, y, size: 9, font, color: rgb(0,0,0) });
      y -= 12;
      page.drawText('• Matériel fourni', { x: 70, y, size: 9, font, color: rgb(0,0,0) });
    }
