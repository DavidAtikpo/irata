  ...
  try {
    ...
    const html = `
      ...
      <head>
        ...
        <title>Toolbox Talk - ${userName}</title>
        <style>
          body {
    ...
    const fileName = `toolbox-talk-${userName.replace(/\s+/g, '-').toLowerCase()}-${record.site.replace(/\s+/g, '-').toLowerCase()}-${new Date(record.date).toISOString().split('T')[0]}.pdf`;
