...
export async function POST(request: NextRequest) {
  try {
    ...
    const invoiceData = {
      ...
      showQr: true,
      paymentStatus: userInvoice.paymentStatus, // Ajouter le statut de paiement
    };

    // Générer le HTML avec toutes les données
    const html = buildHtml(invoiceData, userInvoice.amount, userInvoice);

    // Configuration Puppeteer comme dans le devis
    ...
    // Générer le PDF comme dans le devis
    ...
    // Convertir le PDF en ArrayBuffer comme dans le devis
