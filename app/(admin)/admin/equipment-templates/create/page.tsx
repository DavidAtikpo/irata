...
interface Subsection {
  ...
  isSubtitle?: boolean;  // Si true, c'est un sous-titre (comme "-Fentes et trous accessoires")
  hasGrayBackground?: boolean;  // Si true, applique bg-gray-100 p-1
  isListItem?: boolean;  // Si true, affiche comme <li>
  showStatusButton?: boolean;  // Si false, cache le bouton status (mais garde hasStatus pour les données)
}
...
interface Section {
  ...
  useGridLayout?: boolean;  // Si true, utilise grid 40%/60% comme dans nouveau/page.tsx
}
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorMessage = errorData.error || 'Erreur lors de la création du template';
        alert(errorMessage);
      }
    } catch (error) {
      console.error('Erreur lors de la création du template:', error);
      alert('Erreur lors de la création du template. Veuillez réessayer.');
    } finally {
